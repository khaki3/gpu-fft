...
texture<DATA_TYPE, cudaTextureType2D, cudaReadModeElementType> texRef;
...
__device__ __forceinline__ void sFFT(DATA_TYPE out[], DATA_TYPE a[], size_t n)
{
    for (int i = 0; i < n; i++) {
        float real = 0.0;
        float imag = 0.0;
        float pow = 2 * PI * i / (float)n;

        for (int j = 0; j < n; j++) {
            float powh = fmodf(j * pow, 2 * PI);
            DATA_TYPE d = a[j];
    ...
    if (kernel_id == 1) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            output[id * 8 + threadIdx.x] =
                mul(out[i], tex2D(texRef,
                                  (rem * 8 + threadIdx.x) / (float)16,
                                  row / (float) 4096
                        ));
        }
    }
                             ...
                             cudaEvent_t start, cudaEvent_t stop)
{
    ...
    */
    ...
    // Texture Momory
    DATA_TYPE *twiddle_65536;
    cudaArray *dev_twiddle_65536;

    cudaMallocHost(&twiddle_65536, 65536 * sizeof(DATA_TYPE));

    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 4096; j++) {
            float f = (-2 * PI * i * j) / 65536;
            twiddle_65536[i * 4096 + j] = make_float2(cosf(f), sinf(f));
        }
    }

    cudaChannelFormatDesc channelDesc =
        cudaCreateChannelDesc<DATA_TYPE>();

    cudaCheckReturn(cudaMallocArray(&dev_twiddle_65536, &channelDesc, 4096, 16));

    cudaCheckReturn(cudaMemcpyToArray(
                        dev_twiddle_65536,
                        0,
                        0,
                        twiddle_65536,
                        65536 * sizeof(DATA_SIZE),
                        cudaMemcpyHostToDevice));
    ...
    cudaCheckReturn(cudaBindTextureToArray(texRef,
                                           dev_twiddle_65536,
                                           channelDesc));
