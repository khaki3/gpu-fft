                ...
                DATA_TYPE *dev_data, DATA_TYPE *dev_middle)
{
    ...
    size_t N = DATA_SIZE;
    size_t M = pow(2.0, floor(log2((double)(N - 1)) + 1));
    ...
    for (int l = 0; l <= N-1; l++) {
      float p = (2 * PI * (- 1 / (float)2) * (l * l)) / (float)N;
      h[l].x = cosf(p);
      h[l].y = sinf(p);
    }
    for (int l = M - N + 2; l <= M - 1; l++) {
      h[l] = h[M - l];
    }
    if (M > (2 * N - 2)) {
    ...
    for (int r = 0; r <= M - 1; r++) {
      hCZ[r].x = CY[r].x * hh[r].x;
      hCZ[r].y = CY[r].y * hh[r].y;
    }
    ...
    for (int r = 0; r <= N - 1; r++) {
      float p = (2 * PI * (1 / (float)2) * r * r) / (float)N;
